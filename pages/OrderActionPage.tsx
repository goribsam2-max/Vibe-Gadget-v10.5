        ...
        <div className="space-y-4">
          {orders.map((order) => (
            <motion.div
              whileTap={{ scale: 0.98 }}
              key={order.id}
              onClick={() => navigate(actionName === 'review' ? `/leave-review?productId=${order.items[0]?.id}` : `/track-order/${order.id}`)}
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs font-semibold text-zinc-500 flex items-center gap-1.5">
                    Total: <span className="text-zinc-900 dark:text-zinc-100">৳{order.total.toLocaleString()}</span> • {order.items.reduce((acc, item) => acc + item.quantity, 0)} Items
                  </p>
                </div>
