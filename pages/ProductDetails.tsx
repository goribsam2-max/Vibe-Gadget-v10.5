...
const ProductDetails: React.FC = () => {
  ...
  const images = (product.images && product.images.length > 0) 
    ? product.images 
    : (product.image ? [product.image] : ["https://placehold.co/600x600/png"]);
    ...
    <div className="w-full mx-auto min-h-screen bg-background text-foreground pb-24 lg:pb-0 overflow-x-hidden">
      ...
      <div className="w-full max-w-7xl mx-auto p-4 md:p-8 animate-fade-in">
        ...
        <nav aria-label="Breadcrumb" className="flex items-center text-sm text-zinc-500 dark:text-zinc-400 mb-6 whitespace-nowrap overflow-x-auto no-scrollbar">
          <button onClick={() => navigate("/")} className="hover:text-zinc-900 dark:text-zinc-100 transition-colors">Home</button>
          <ChevronRight className="h-4 w-4 mx-1 shrink-0" />
          <button onClick={() => navigate("/all-products")} className="hover:text-zinc-900 dark:text-zinc-100 transition-colors">Products</button>
          <ChevronRight className="h-4 w-4 mx-1 shrink-0" />
        ...
        <main className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12">
          ...
          <div className="flex flex-col">
              ...
              {mysteryOffer ? (
                 <div className="flex justify-between items-center bg-zinc-900 border border-zinc-800 p-4 rounded-xl text-white">
                    <div>
                      ...
                      <div className="flex items-center gap-3">
                         <span className="text-4xl font-bold tracking-tighter text-zinc-700 dark:text-zinc-300">৳{mysteryOffer.discountPrice.toLocaleString()}</span>
                         <span className="text-sm line-through text-zinc-500 font-semibold">৳{product.price.toLocaleString()}</span>
                      </div>
                    </div>
                    <Icon name="gift" className="text-3xl text-zinc-700 dark:text-zinc-300 animate-pulse hidden sm:block" />
                 </div>
              ) : (
                <div className="flex items-end gap-3">
                  ...
                  {(product as any).freeShipping && (
                     <span className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 dark:text-zinc-700 dark:text-zinc-300 mb-1 ml-2 bg-zinc-200 dark:bg-zinc-700 dark:bg-emerald-900/30 px-2 py-0.5 rounded-full">+ Free Shipping</span>
                  )}
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 my-2 z-10 relative">
              {product.stock <= 0 ? (
                ...
                <>
                  ...
                  <Button onClick={() => addToCart(true)} size="lg" className="flex-[1.5] gap-2 rounded-full py-6 text-base shadow-lg shadow-emerald-500/20 bg-zinc-800 dark:bg-zinc-200 hover:bg-emerald-700 text-white border-none">
            ...
            <div className="flex flex-wrap gap-2 my-6">
                ...
                <StatusBadge leftIcon={ShieldCheck} leftLabel="100% Authentic" />
            </div>
               ...
               <div className="flex justify-between items-center p-4 bg-white dark:bg-zinc-900/80 backdrop-blur-sm rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm">
                  <div className="flex items-center gap-4">
                      <Avatar className="h-12 w-12 border-2 border-emerald-100 dark:border-emerald-900/50 bg-zinc-100 dark:bg-zinc-800 dark:bg-emerald-900/20">
                          <AvatarFallback className="bg-zinc-200 dark:bg-zinc-700 text-zinc-950 dark:text-zinc-50 font-bold dark:bg-emerald-900/30 dark:text-zinc-700 dark:text-zinc-300">VG</AvatarFallback>
                      </Avatar>
                      <div>
                  ...
                  <Button variant="ghost" size="sm" className="text-zinc-800 dark:text-zinc-200 dark:text-zinc-700 dark:text-zinc-300 text-xs font-bold" onClick={() => navigate('/all-products')}>
                      Visit Store &rarr;
                  </Button>
              </div>
            </div>
